...
- не выбирает картинки из исходных массивов Google Sheets;
- не форматирует бизнес-данные, если они могут быть подготовлены до отрисовки;
- получает для карточек уже подготовленные поля вроде `mediaHtml`, `badgeHtml`, `priceFormatted`;
- использует `data-*` атрибуты для делегирования событий в `app.js`;
- управляет DOM-состоянием шторки товара: открыть, закрыть, обновить количество, отрисовать слайды;
- рисует счетчики избранного и корзины;
- рисует экран корзины: пустое состояние, строки товаров, количество и итог;
...
- хранит frontend-состояние: каталог, избранное, корзину;
- хранит состояние открытой шторки: текущий букет, количество, слайды, индекс слайда;
- хранит текущий раздел нижней навигации: `home`, `fav`, `cart`, `delivery`;
- управляет состоянием корзины: добавить, изменить количество, удалить;
- формирует готовые строки корзины для `ui.js` и считает итог;
- подготавливает данные каталога для `ui.js`;
- использует делегирование событий через `event.target.closest()`;
- не генерирует HTML напрямую, если это можно вынести в `ui.js`.
...
## Загрузка фото и согласование букета

Для согласования букета перед доставкой используется локальное хранилище на сервере Beget и отправка уведомлений с фото в Telegram.

Важно: Telegram-ссылки на файлы из Bot API живут ограниченное время, поэтому Telegram не используем как постоянное хранилище фотографий. Постоянная ссылка хранится на нашем сервере, а Telegram используется как канал уведомления.

Сценарий:

1. Флорист загружает фото готового букета через Mini App.
2. Backend сохраняет файл локально и переводит заказ в `PHOTO_REVIEW`.
3. Backend отправляет клиенту в Telegram сообщение с фотографией букета.
4. Клиент открывает Mini App и нажимает "Принимаю" (`READY`) или "Исправить" (`REVISION`).
5. Если клиент выбрал "Исправить", заказ возвращается флористу с комментарием `revision_note`.

Техническая реализация backend:

- хранение: локальная папка `backend/uploads/orders/`;
- папка `uploads/` не коммитится в Git;
- раздача статики в `server.js`: `app.use('/uploads', express.static(path.join(__dirname, 'uploads')));`;
- библиотека для приема файлов: `multer`;
- разрешены только изображения;
- максимальный размер файла: 5 MB;
- имя файла генерирует сервер, например `order-123-1720000000000.jpg`;
- в `orders.photo_url` сохраняется постоянная публичная ссылка на файл.

Изменения в базе данных:

- `orders.photo_url VARCHAR(500) DEFAULT NULL`;
- `orders.revision_note TEXT DEFAULT NULL`;
- в `orders.status` и `order_status_history` добавляются `PHOTO_REVIEW` и `REVISION`.
...
- интерфейс флориста использует `<input type="file" accept="image/*" capture="environment">`;
- клиентский экран доставки показывает фото, поле комментария и кнопки "Принимаю" / "Исправить";
- при `READY` уведомляем курьеров;
- при `REVISION` уведомляем флориста и сохраняем комментарий клиента.
...
Флорист загрузил фото собранного букета, заказ отправлен клиенту на согласование.

Действие:

- сохранить фото на сервере;
- отправить клиенту Telegram-сообщение с фотографией;
- предложить открыть Mini App для принятия или отправки правок.

### REVISION

Клиент попросил исправить букет.

Действие:

- сохранить комментарий клиента в `revision_note`;
- уведомить флориста о доработке;
- после исправления флорист снова отправляет фото на согласование.

### READY

Клиент принял фото букета, заказ готов к доставке.
...
- florist может менять `ASSEMBLING -> PHOTO_REVIEW`;
- florist может менять `REVISION -> PHOTO_REVIEW`;
- client может менять `PHOTO_REVIEW -> READY`;
- client может менять `PHOTO_REVIEW -> REVISION`;
...
- Фото согласования храним локально в `backend/uploads/orders/`, а Telegram используем для уведомления клиента через `sendPhoto`.
- Telegram file URL не используем как постоянное хранилище, потому что такие ссылки временные.
